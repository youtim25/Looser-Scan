...
def dedupliquer_images(images):
    ...
    for img in images:
        ...
        nom_base = img.rsplit('.', 1)[0].strip()  # Nom sans extension
        
        # Si c'est une image qui ne commence pas par un chiffre (thumbnail)
        if not img[0].isdigit():
            images_uniques[nom_base] = img
            continue
        
        # Pour les images numérotées, garder la meilleure version
        if numero not in images_uniques:
            images_uniques[numero] = img
        else:
            # Préférer .webp, sinon garder l'existant
...
def generer_donnees():
    ...
    for index, dossier in enumerate(dossiers):
        ...
        try:
            ...
            # DÉDUPLICATION : éliminer les doublons .jpg/.webp
            ...
            print(f"✓ {dossier} : {len(images_list)} images (après déduplication)")
